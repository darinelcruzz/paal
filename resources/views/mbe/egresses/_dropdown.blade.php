<dropdown color="{{ $color }}" icon="cogs">
	...
	<ddi icon="edit" to="{{ route('mbe.egress.edit', $egress) }}" text="Editar folio"></ddi>
    ...
    @else
    	...
    	<ddi icon="upload" to="{{ route('mbe.egress.replace', $egress) }}" text="Subir factura"></ddi>
    ...
    @if ($egress->status != 'pagado')
        <ddi icon="usd" to="{{ route('mbe.egress.pay', $egress) }}" text="Pagar"></ddi>
    @endif
